...
import { ChatHistory, ChatMessage, MessageRole, CLICommand, CLIState, APIError, RateLimitError, NetworkError, TokenStats } from '../types';
...
export class CLI {
  ...
  private async handleMessage(content: string): Promise<void> {
    try {
      ...
      const [response, tokenUsage] = await this.client.sendMessage(this.history.messages);
